<template>
  <div>
    <div class="power-header">注册渠道权限列表</div>
    <div class="power-body">
      <div class="total-power-list">
        <ul class="item-ul open">
          ...
          <div v-if="Channels.length==0&&!isNoData">正在获取注册渠道权限列表...</div>
          <div v-if="Channels.length==0&&isNoData" style="color:red;">注册渠道权限列表为空!</div>
        </ul>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getPermissChannels() {
      ...
      services.user.getActorChannels(params).then(
        response => {
          if (response.code == 401) {
            var state = response.state[0]
            if (response.state.length == 0 || state.length == 0) {
              this.isNoData = true;
            }
            this.Channels = this.formatChannels(state);
          } else {
            showPrompt({
    ...
    getChannelLevel(channels) {
      for (var i = 0; i < channels.length; i++) {
        if (
          channels[i][this.channelConfig[3].grand_id] &&
          channels[i][this.channelConfig[3].parent_id] &&
          channels[i]['channel_id']
        ) {
          return 3;
        } else if (channels[i]['channel_id']) {
          return 1;
        }
        return 0;
      }
    ...
    formatChannels: function (channels) {
      if (this.currentGame) {
        ...
        // var level = 1
        // if (this.channelConfig[systemId] && this.channelConfig[systemId].level) {
    ...
    addGrantChannels: function () {
      ...
      services.user.setActorChannels(params).then(
        function (response) {
    ...
    getOneLevel: function (channel) {
      ...
      channel.forEach(item => {
        ...
        first[obj.channel_id] = obj;

        let second = first[obj.channel_id].children;
        if (!second.hasOwnProperty(item.channel_id)) {
          second[item.channel_id] = {
            id: item.channel_id,
            name: item[this.channelConfig[1].name].trim(),
            isowner: item.isowner == "t" ? true : false
          };
        }
        first[obj.channel_id].length++;
      });
